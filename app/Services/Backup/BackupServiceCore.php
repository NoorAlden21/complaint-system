...
final class BackupServiceCore
{
    public function __construct(
        private BackupLogRepositoryInterface $backups
    ) {
    }
    ...
    {
        if ($backupLog->status !== 'success') abort(400, 'Cannot download a failed backup.');
        if (!$backupLog->disk || !$backupLog->path) abort(404, 'Backup file information is missing.');
        if (!Storage::disk($backupLog->disk)->exists($backupLog->path)) abort(404, 'Backup file not found on disk.');
